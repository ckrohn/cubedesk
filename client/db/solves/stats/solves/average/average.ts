...
export function getAverageOf(solves: Solve[],
                             solveIndex: number = 0, averageOf: number = 5): number {
    if (!solves || !solves.length) {
        return 0;
    }

    let list: Solve[] = solves.slice(solveIndex, solveIndex + averageOf);
    if (list.length < averageOf) {
        return -1;
    }

    let times = list.map(s => s.time);
    const min = Math.min(...times);
    const max = Math.max(...times);

    // TODO: Only filter out first and last occurrence
    times = times.filter(t => t > min && t < max);
